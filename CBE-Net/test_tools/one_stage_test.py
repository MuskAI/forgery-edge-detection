    ...
    def read_test_data(self, src_data_dir, output_dir, name_list=None):
        ...
        try:
            for index, name in enumerate(tqdm(image_name)):
                img = Image.open(os.path.join(test_data_path, name))
                ...
                cv.imwrite(os.path.join(output_dir, '{}.png'.format(name)), output)
...
class TestDatasets(Datasets):
    def __init__(self):
        ...
        self.datasets_path = {
            'root': '/home/liu/haoran/test_results',
            'columbia': None,
            'coverage': None,
            'casia': None,
            'my-protocol': None
        }
        super(TestDatasets, self).__init__(model_name='demo', using_data=self.using_data,
    ...
    def infer_all_dataset(self):
        for idx, item in enumerate(tqdm(self.datasets_dict)):
            ...
            img_path = os.path.join(self.datasets_path[item]['path'], item2)
            self.one_stage_infer.read_test_data(src_data_dir=self.datasets_path[item]['path'],
                                                output_dir=self.datasets_dict[item]['save_path'],
                                                name_list=self.datasets_dict[item]['names'])

    def get_model(self):
        model_path = self.model_zoo()['HDG-final']
    ...
    def model_zoo(self):
        model_dict = {
            'HDG-final': {'model_py': getattr(import_module('model.unet')), 'model_path':''},
        }
        return model_dict
