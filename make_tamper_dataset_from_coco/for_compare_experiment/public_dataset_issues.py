    ...
    def coverage(self, in_path_src=None, in_path_gt=None, save_path=None):
        ...
        for src_name in src_list:
            if 't' not in src_name:
                continue
            _src_list.append(src_name)
            gt_name = src_name.replace('t', 'forged')
            print('The gt_name is :', gt_name)
            if gt_name not in gt_list:
                print('src:[%s] with gt:[%s] not match' % (src_name, gt_name))
                unmatched_list.append(gt_name)
        ...
        for item in src_list:
            if item.replace('t', 'forged') not in unmatched_list:
                matched_list.append(item)
                shutil.copyfile(src=os.path.join(in_path_src, item), dst=os.path.join(save_path + '\\src', item))

        # 2 generate gt from gt_list and save it
        for index, gt_name in enumerate(matched_list):
            gt_name = gt_name.replace('t', 'forged')
            ...
            ################
            out_gt = Image.fromarray(out_gt)
            out_gt.save(os.path.join(save_path + '\\gt', gt_name))
            print('\r', 'gt generate process: {:d}/{:d}'.format(index, len(gt_list)), end='')

        return True
    def realistic(self, in_path_src=None, in_path_gt=None, save_path=None):
        # 0 有效性判断
        PublicDataset.__casia_path_check(self, in_path_src, in_path_gt, save_path)
        ...
        for item in src_list:
            if item.split('.')[0] +'.PNG' not in unmatched_list:
                matched_list.append(item)
                # shutil.copyfile(src=os.path.join(in_path_src, item), dst=os.path.join(save_path + '\\src', item))

        # 2 generate gt from gt_list and save it
        for index, gt_name in enumerate(matched_list):
            gt_name = gt_name.split('.')[0] +'.PNG'
            gt = Image.open(os.path.join(in_path_gt, gt_name))
            gt = np.array(gt)
            if gt.shape[-1] == 3 or gt.shape[-1] == 4 or gt.shape[-1] == 2:
                gt = gt[:, :, 0]
                gt = np.where(gt > 0, 1, 0)
            else:
                pass
            ...
            try:
                out_gt = Image.fromarray(out_gt)
                out_gt.save(os.path.join(save_path + '\\gt', gt_name.replace('.PNG','.png')))
...
if __name__ == '__main__':
    #
    in_path_src = r'D:\lab\realistic-tampering-dataset\data-images\combine\src'
    in_path_gt = r'D:\lab\realistic-tampering-dataset\data-images\combine\mask'
    save_path = r'G:\3月最新数据\public_dataset\realistic5'
    ...
    PublicDataset().realistic(in_path_src, in_path_gt, save_path)
